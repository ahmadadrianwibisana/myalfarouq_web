                    ...
                    <div class="row">
                        ...
                        <div id="private_trip_field" class="col-md-6" style="display: none;">
                            <div class="form-group">
                                ...
                                <select name="trip_id" id="private_trip_id" class="form-control">
                                    <option value="">Pilih Private Trip</option>
                                    @foreach ($privateTrips as $item)
                                        <option value="{{ $item->id }}" {{ old('trip_id') == $item->id ? 'selected' : '' }}>{{ $item->nama_trip }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
...
<script> 
    function toggleTripType() { 
        ...
        if (tripType === 'open_trip') { 
            openTripField.style.display = 'block'; privateTripField.style.display = 'none'; 
            ...
            document.getElementById('open_trip_id').disabled = false; 
                ...
                privateTripField.style.display = 'block'; openTripField.style.display = 'none'; 
                document.getElementById('open_trip_id').disabled = true; 
                document.getElementById('private_trip_id').disabled = false; 
            } 
                else { 
                    openTripField.style.display = 'none'; privateTripField.style.display = 'none'; 
                } 
    } 
</script>
@endsection